...
@section('page_title',__('Add').' '.__(kebab2Title($content)))
	...
	<div class="main-content">
		<section class="section">
			<div class="section-header">
				<h1>@lang('Add') @lang(kebab2Title($content))</h1>
				<div class="section-header-breadcrumb">
					<div class="breadcrumb-item active">
						<a href="{{ route('admin.home') }}">@lang('Dashboard')</a>
					</div>
					<div class="breadcrumb-item">@lang('Add') @lang(kebab2Title($content))</div>
				</div>
			</div>
						...
						<div class="col-lg-12">
							<div class="card mb-4 card-primary shadow">
								<div
									...
									<h6 class="m-0 font-weight-bold text-primary">@lang('Add') @lang(ucfirst(kebab2Title($content)))</h6>
									<a href="{{route('content.index',$content)}}"
								...
								<div class="card-body">
									@if(array_key_exists('language',config("contents.$content")) && config("contents.$content.language") == 0)
										<form method="post" action="{{ route('content.store', [$content,0]) }}"
										...
										<div class="tab-content mt-2" id="myTabContent">
											@foreach($languages as $key => $language)
												<div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
													...
													<form method="post"
														  action="{{ route('content.store', [$content,$language->id]) }}"
														  enctype="multipart/form-data">
														@csrf
														@method('put')
														<div class="row">
																	...
																	<div class="col-md-12">
																		<div class="form-group">
																			...
																			<div class="input-group input-group-sm">
																				...
																				<div
																					class="invalid-feedback">@error($name.'.'.$language->id) @lang($message) @enderror</div>
																			</div>
																		</div>
																	</div>
																@endif
	...
	<script>
		...
		$(document).ready(function () {
			$('input[type="file"]').each(function (key, selector) {
				...
				$.uploadPreview({
					input_field: `#${attrId}`,
					preview_box: `#${attrPreviewId}`,
					label_field: `#${attrLabelId}`,
